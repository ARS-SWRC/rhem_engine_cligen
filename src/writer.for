...
C------------------------------------------------------------------------------
      ...
      subroutine writer ( sfile,  idevent, iday, imon, iyear,
     &   Rainfall, Infil, Runoff, PeakR, SoilLoss, sloss, deposition,
     &   suspended, stmdur, prcp, mxint )
...
C------------------------------------------------------------------------------
      ...
      real    pwse, pwse_neg, pwse_pos, pwsi, pwss, pwso, stmdur, prcp,
     &        mxint, infil
      integer i, j, l, m, n, k, file1, file2, ierr, idevent, iday,imon, 
     &        iyear, flag, norphs, ii, outr, idl, fintgs, file_unit !, ltyp, lfh 
...
C
      ...
      Character(Len=2) :: flag2print
      Character(Len=150) :: fields(11)
...
C
      if(rsoil) then
C        write(file1,808) thinu, satin, thwilt, satwl, thfc, satfc
        select case ( infmode )
          case ( 1 )
C            write(file1,810)
          case ( 2 )
...
C                                                        write sediment outflow
      ...
      call write1 (buffer(1:m))
C----------------TEST 20NOV2012 -----------------------------------------------
C ----- Mariano Hernandez 13Nov2012 -------------------------------------------
      if (suma) then
          runoff_volume_mm = qbal(10)/coarea*conv
C          if ( time_runoff_dur .lt.  0. ) time_runoff_dur = 0.
          if ( qbal(10)/coarea*conv .le. 1.e-3 .or.
     &                                time_runoff_dur .le.  0.  )then
              runoff_volume_mm = 0.
              qp2 = 0.
              time_runoff_dur = 0.
              pwso=0.
          ...
          if ( pwso*10000./qbal(1) .ge. abs(pwse_neg*10000./qbal(1) ) )
     &    then
          
          write(23, 1956) idevent, iday, imon, iyear, prcp*1000.,
     &                    mxint*3600.*1000., stmdur/60.,
     &                    runoff_volume_mm,qp2, time_runoff_dur/60.,
     &    ( pwso*10000./qbal(1) )/1000., (pwso*10000./qbal(1))/1000.
          else
              
          write(23, 1956) idevent, iday, imon, iyear, prcp*1000.,
     &                    mxint*3600.*1000., stmdur/60.,
     &                    runoff_volume_mm,qp2, time_runoff_dur/60.,
     &    ( pwso*10000./qbal(1) )/1000., 
     &    ( abs( pwse_neg*10000./qbal(1) ) )/1000.
          end if
              
      end if
          
C------New Approach 18Jan2013 -------------------------------------------------
C------Store results in the arrays
      Rainfall = qbal(2)
      Infil = qbal(6)
      Runoff = qbal(10) ! m^3
      PeakR = qp2
      SoilLoss = pwso*10000./qbal(1)
      deposition = pwse_pos*10000./qbal(1)
      SLoss = pwse_neg*10000./qbal(1)
      suspended = pwss*10000./qbal(1)
C------------------------------------------------------------------------------      
C        else
C          write(22, 1956) idevent, iday, imon, iyear,
C     &                    qbal(1), qbal(2), qbal(6), qbal(10),
C     &                    qp2, twso, slab2
C        end if
        
1956    format(1x,4i7, 8F12.4)
...
C      if(adjnote .eq. '(a)') write(77,'(18x,"[",a3," = adjusted for mean 
C     & total infiltrating width ]")')adjnote
...
C------------------------------------------------------------------------------
      ...
      entry event (sumbal, idevent, iday, imon, iyear)
...
C        write (file1,'(" Number of elements processed [",i4,"] does not 
C     &equal NELE [",i4,"] in input file. ")') nelt, nele
      else
C        write(file1,'(i4," elements Processed"/)') nelt
      end if
...
C            write (file1, 75) (pso(i), i = 1, nps)
C75          format ('   % of total yield', 5(6x, f6.2))
...
C
        call blanks
        call blanks
C        write(file1,'(20x," Tabular Summary of Element Hydrologic Compon
C     &ents"/)')
...
C                                        put headings into outfile
        do i=1,3
C          write(file1,trim(fmthl)) head(i),
C     &        (hedstr(i,jc)(1:je(jc)),jc=1,nco)
        end do
...
C                     first go thru list and report planes, injects, and urban els.
        do jp = 1,ltab
          ...
          if(itu .eq. 0 .or. itu .eq. 6 .or. itu .eq. 5) then
            ...
            if(itu .eq. 5) then
C              write(file1,fminj) charid,sumtab(jp)%volro,
C     &           sumtab(jp)%ropeak
            else
...
C              write(file1,trim(fmtdat)) charid,(dval(i),i=1,nco)
            end if
          end if
        end do
...
C            write(file1,trim(fmtdat)) charid,(dval(m),m=1,nco)
          end if
        end do
