...
C------------------------------------------------------------------------------
     ...
     &           inflow(12), nu, upr, outr, ngage, nr, nq, nzro=0
...
C
      ...
      real,dimension(20) ::  topw, slp, fav, qt, afac, h1avg, xc
...
C      if(DIAG) write(99,'(" Diagnostics for plane ",i3)')ID

      msg(1:16) = typname(0)
      write (msg(15:20), '(i4)') id
C
      do ms = 1, 5
...
C** 22/4/03  dx check
      ...
      if(dx .ge. dxlim) then
C        write(files(3), 1990) id, dx, chl!  , dxlim
C 1990 format(/"  Plane ",i3,":  based on length and parameter CLEN,"/
C     &"  the numerical increment of ",f6.1,1x,a2," is too large for",/
C     &"  realistic numerical solution of the flow equation, and may "/
C     &"  give  misleading results."/)
      end if
      
      If(diag) then
C        write(99,393) xleng,dlab,nk,dx,dlab
C 393    format(' Plane is ',f8.1,1x,a2,' long with ',I2,' nodes and DX o
C     &f',f8.4,1x,a2/)
      end if
 !     
C                                                                         WIDTH
...
C!! added balance component for upstream info.  RES 03/12/02
      smup = 0.
C -- Minimum simulation time Calr Unkrich 19Dec2012
      if (up) then
...
C------Mariano Hernandez 28Apr2013 --------------------------------------------
C
C
C
      if ( reldif .gt. 1.e-10 )then
            time_hyd_min(i) = time
      end if
C------------------------------------------------------------------------------        
C
          ...
          if (unif) then
            ...
            do j = 2, nk
              ...
              if (h2(j) .le. 0.) then
                ...
                a2j   = h2(j) * ams
                q2(j) = alpha(j) * a2j**p(j) /wpc**(p(j) - 1.)
...
C  Only gets to this point if unif is false and ql is negative (hydrograph recession)
...
C                                                iterative solution for h2(j+1)
              ...
              if (ierr .gt. 0) then
C                if(diag) write(99,*) jp1, hmin,h2jp1,ql(j,1),ql(jp1,1)
C                close(99)
...
C            write(99,'("  h2",8g12.4)') (h2(j),j=1,nk)
C            write(99,'("  q2",8g12.4)') (q2(j),j=1,nk)
C           if(amr .gt. 0.)write(99,'(" tw ",8g12.4)') (topw(j),j=1,nk)
C  101 format("    step  t(min)  sums(L^2): RF       Infil        Excess 
C     &       OutQ         Stor         Bal")
C            write(99,101)
C            write(99,'(" ba:",i4,8g13.5)') i-1,time/60.,sumrf*xleng,
C     &  smin, smql, qbal(10), storh, sterr
C
          end if
...
!**
C ----------Mariano Hernandez 29Apr2013 --------------------------------------- 
C           if (h2(nk) .gt. 1.e-7) time_hyd_max(i) = time
       if ( q2(nk) .gt. 1.e-7) then
            time_hyd_max(i) = time
       else
           time_hyd_max(i) = 0.
       end if
...
100   continue
C Mariano Hernandez 04Apr2013 
C calculate duration of runoff    
      time_pond = minval( time_hyd_min, MASK=time_hyd_min .gt. 0. )
      time_runo = maxval( time_hyd_max, MASK=time_hyd_max .gt. 0. )
      if (time_runo .le. 0.) time_runo = 0.
      time_runoff_dur = time_runo-time_pond
      if (time_runoff_dur .le. 0.) time_runoff_dur = 0.
...
C                                                         pass values to writer
      ...
      call qwrt (id, 1, msg, qpw, tp, vin, coarea,
     &            outq, 0, outr, qrmax)
...
C   equation for flow on a sloping plane.  Includes microtopog. geometry
     ...
     &                pturb, laminar
      ...
      if (h2jp1 .ge. h2c) then
        ...
        q2(jp1) = alpha(jp1) * a2jp1**p(jp1)/ wpc**(p(jp1) - 1.)  !m^2/s
