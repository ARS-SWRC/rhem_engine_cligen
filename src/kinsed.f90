...
! --- BEGIN Marinao Hernnadez 11Dec2013      
!      CALL getr4('KI', 0, ki, ierr) ! interrill erodibility for RHEM
      CALL getr4('KSS', 0, ki, ierr) ! interrill erodibility for RHEM
! --- END Mariano Hernandez 11Dec2013      
...
!---------------------------------------------------------------------------------------------------
    ...
    SUBROUTINE kinsed(indx, dt, dte, ql, a2, q2, topw, rf, qup, qt, time)
      ...
      INTEGER :: indx
      REAL    :: dt,dte,ql(20,2),a2(20,nchan),q2(20,nchan),topw(20,nchan),rf,qup(10,2),qt(20),time, timepond, timerunoff
...
!       CALL kinsed_rhem(indx, dt, dte, ql, a2, q2, rf, qup, time, timepond, timerunoff)
      END IF
...
!---------------------------------------------------------------------------------------------------
    ...
    SUBROUTINE sedbf(a2, q2, topw)

      REAL :: a2(20,nchan),q2(20,nchan),topw(20,nchan)

      IF(sed_model == 1) THEN ! Smith
        CALL sedbf_smith(a2, q2, topw)
      END IF
