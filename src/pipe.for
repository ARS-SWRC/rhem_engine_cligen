...
!-------------------------------------------------------------------------------
      integer upr, upq, outq(2), outr, nq
      character msg*18
...
C 1990 format(/"  Pipe ",i3,":  based on its length and parameter CLEN,"/
...
C set up min angle parameters:
      ...
      wz = thz*diam/2.
C
C      if(diag) write(99,'(" al, p : ",2g12.4)') alpha, power
...
!-------------------------------------------------------------------------------
      if(sed) then
        ...
        call sed0(msg(ms:18), inflow, nu, 0, nk, omega, dx, slp, 1.)
...
! iterative solution for th2(j+1)
              ...
              if (ierr .gt. 0) then
C                write(99,'(2i3,4g12.4)') j, ierr, th2(j),th1(jp1),th2jp1
C                call errxit(msg(ms:18),'no convergence (errth2)')
              else
               itotl = itotl - ierr
              end if
...
C  recompute with new subinterval
      ...
      ! check for peak flow
        ...
        ! reset variables for next time step
          ...
          if(diag) then
C            write(99,801)'pipe',i,itotl,qub
C            write(99,800)' a2 ',(ar2(j),j=1,nk)
C            write(99,800)' q2 ',(qr2(j),j=1,nk)
C            write(99,800)'th2 ',(th2(j),j=1,nk)
          end if
C 800  format(a4,8g12.4)
C  801 format(a4,2i5,5g12.4)
