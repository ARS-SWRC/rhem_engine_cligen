...
C
     ...
     &      pb2(2), eps1(2), eps2(2), erfi(2), dtu, trp, i, dbg, infm
...
C                                         capillary drive includes surface head
            ...
            if( apu .le. 0.) then
              ...
              if(op) iunit = 99
C              write(iunit,"(' APU negative in INFILT: '/,i3,(7g12.4))") 
C     &           j, g1(i), zfl(i), trp, ht, gu(i), thiu(j,i), defs
C              stop    ' negative value of cap. drive in INFILT for this
C     &element '
            end if
C  
            rhc = 1.0
...
C!!             
              fap =  favdI( infm, apu, vmin, viu, alf, dtu)/dtu
C
...
C                write(calldat,'("r",i2,"/",i2,i3)')j,nk,itm
...
C                thiur = thur  ! temporary
                f2t = fi(j,i)  
C!!
C                write(calldat,'("p",i2"/",I2,i3)')j,nk,itm
...
C                  write(99,'(" rerr:",a6,i2,4g12.4)') fmode, j, f2t 
                end if
C
...
C!! new 12/02: value of storage until prewet pulse is passed:
              capr = (ths1(i) - tho(J,i))*zb(j,i)
              if(subcon(i) .and. lowet(j,i)) then
               fi(j,i) = 0.
               fv(j,i) = fs(j,i)
               qki(j,i) = 0.
C!! new test for negligible fi
...
C                write(calldat,'("b",i2,"/",i2,i3)')j,nk,itm
C                                                               estimate rising
...
C   iterate: try f2b with limits vl(max) and f2m(min): 
 ...
 !
            ...
            if (ierr .gt. 0) then
              ftav = (f2b - f2l)/dtu
C              write(77,788)ierr, j, t(i)/60., f2l, trial, f2m, vl, f2b,  
C     & vmu*po, ftav*po, vst
C 788  format(" infilt err:",2I3,f7.1,8g12.4)
C              stop ' error - no convergence (errvf) base'
...
C
            if(diag .and. j .eq. jd) then
              ftav = (f2b - f2l)/dtu
C             write(99,789) j, trial, f2l, f2m, vl, f2b, ftav*po, fil*po
C     &          vmu*po,
C 789  format(" t, etc.:",I5,8g12.4) !,f7.1
            end if
...
C                   iunit = 77   ! output file should always be opened
                   ...
                   if(op) iunit = 98
C            write(iunit,"(' APU negative in infilt-rw: ',2i3,(6g13.5))") 
C     &           j, i, trp, zb(j,i),ht, gb, gu(i), thiv 
C                 stop ' negative value of cap. drive in INFILT/rw for th
C     &is element '
                  end if
...
C                                                                   rewet pulse
      if((f2m .lt. 0. .or. f2l .lt. 0. .or. vl .lt. 0.) .and. diag) then
C        write(99,'(" p",i3,6g13.4)') j, fs(j,i), fupr, ffl(j,i), fv(j,i)
C     & , f2l
        end if
...
C                                        ervf uses vmu for the effective Ksat
                if (ierr .gt. 0) then
C         write(77,969) j,ref*po, f2l,f2p,f2m,vl,trial,vns,tsn
C                  stop ' error - no convergence (errvf) rewet'
                end if
                if (f2p .lt. f2l) then
C         write(77,969) j,ref*po, f2l,f2p,f2m,vmu,vl,bav*po,vqki*po
C  969  format(i3,8g13.4)
C                  stop ' error - infilt/iter: Fnew less than Flast'
                end if
...
C 
                if(diag .and. j .eq. jd) then    
C        write(99,790) j, vl,f2p,fv(j,i), ht, ref*po,
C     &  rwav*po, apu, famx*po
C 790  format(" t, rew:",I5,8g12.4)
                end if
              end if    
...
C          write(99,'(/" net inf:", g12.4)') sumfs
        end if
C  198 format(" lowet topfil    qki      tho         thiu         defl      
C     &    rxv"/3x,L2,4x,L2,5g12.4)
C  199  format(" infl:t  j iter  mode    rf         f          Ks(eff)      
C     &   Fs         Fv          F(1)        alph")
C                                                        
      if(itm .ge. itlim ) then
        sumtab(ltab)%flowr = 0.
C        if(diag .and. sk1(i) .gt. 0.) 
C     &         write(99,'(" Tot fs:"/,(8g12.4))') (fs(j,1),j=2,nk)
        nkm = nk-1
        if(subcon(i)) then
          finlot = 0
C       
...
C          if(diag)   write(99,'(" fl:",8g12.4)') (ffl(j,1),j=2,nk)
        end if
...
C --- Mariano Hernandez 11Dec2013        
C        call getr4 ('KS', 2, sk2(i), ierr)
...
C
      if(diagn) then
C        write(99,999) vmu*po, cumcm(i), g1(i), g2(i), g2u(i),gu(i),cv(i)
C        if(rsoil) write(99,998) thin(i),thin2(i),ths1(i),ths2(i)
C     &   ,vmin2(i), rc(i)
      end if
C 999  format(2x,' vmu          cumc         G1          G2         G2U 
C     &        GU         CV(K)'/8g12.4)
C 998  format(2x,' thi          thi(2)      ths1        ths2'/8g12.4)
      return
...
C                        ! gets fv2 with spatial capacity function:
        call fcapv (infm, F2n, fv2, ifn, dfdi) 
...
C               write(99,98) trp, ratl, f2n, xtermn, ferr, dferr
C  98  format( 8g13.5)
             end if
           end if
C
...
C
      ...
      if(thwt .gt. ths .or. thwt .lt. thr) then
        ...
        inquire (99, opened = op)
        if(op) iunit = 99
C        write(iunit,'(" rect bounds ",i3,5g13.4)') 
C     &    il, trp, thr, thu,ths, gup 
      end if
...
C          write(77,277) origin,nloop,  fv,fl,thr,thit,thu,thls,rf/st,
C     &    fk1, fk2, fk3, fk4, th2, th3, th4,   dfav  !,  avg0, avg1, avg3
C 277  format(1x,a9,i3,7g12.4/7g12.4/7g12.4)
C          this = 0.5*(thit+thr)
          exit
        end if
C
...
C          write(99,'(a9,i3,4f9.4/5g13.4/10x,4g12.4)')  origin, nloop,
C     &     dtt,t,ft,rf, this, fk1, fk2, fk3, fk4,  th2, th3, th4, dfac 
...
C        if(dbg) write (99,'(a9," rect out of bounds ",/i4,7g12.4)') 
C     &   origin,  nloop,dtu,this,thit,thr,rf,fv,ft
...
C
        ...
        if(abs(hs -.05) .le. 1.e-10) then
          ...
          flams = GP*(1.- dp*(1. +(Aparm(a)/(hs-.05))**be)**(-1./be)) ! GofP; AParm
        end if
C
      else
C
...
C
      if(str .eq. 'diag') then
C        write(77,'(" funk",5g12.4/5g12.4)')funk, thwu, thwt, fu, zuinv, 
C     &   tm1, tm2, gu, r
      end if
C 
      return
...
C 
        if(gef .le. 0.) then
           ...
           inquire (98, opened = op)
           if(op) iunit = 98
C          write(iunit,"(' Gu negative in setg: '/a6,i3,(5g13.4))") 
C     &           trace, i, gef, pb1(i),thi, g1(i), ht 
C          stop ' negative value of cap. drive in setg for this element '
        end if
      else
...
C          thwf = thm2(i)
          ...
          if(gef .le. 0.) then
           ...
           if(op) iunit = 98
C           write(iunit,"(' Gu negative in setg: '/a6,i3,(5g13.4))") 
C     &           trace, i, gef, thi, gfm, zb, zfl(i)
C          stop ' negative value of cap. drive in setg for this element '
          end if
...
C                                             possible lower layer control
          ...
          if (g2(i) .ge. 0.0001) then
            ...
            if( apu .le. 0.) then
              ...
              inquire (98, opened = op)
              if(op) iunit = 98
C          write(iunit,"(' APU negative in INFILT/s: '/a6,i3,(5g13.4))") 
C     &           trace, i, gef, thi, thi2, thm2(i), gef 
C              stop ' negative value of cap. drive in setg for this
C     &element '
            end if
          end if
        end if
