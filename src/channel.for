...
C------------------------------------------------------------------------------
     ...
     &        leng, upq,  i, inflow, idup, upr, idlat, outr, nr, nq  
...
C                                                                    IDENTIFIER
      ...
      if(DIAG) then
        jd = 2
C        write(99,'(" Diagnostics for channel ",i3)') ID
      else
        jd = 100
      end if
C
...
C 1990 format(/"  Channel ",i3,":  based on length and parameter CLEN,"/
...
C     &"  give  misleading results."/)
 ...
 !     
C      if(diag) write(99,'(" length div. into ",i2," segments of ",f8.2,
C     &1x,a2," each.")') nkm, dx, dlab
...
C        if (slope(2,1) .gt. slope(1,1)) then
C          zaj = nk - 1
        else
C          zaj = 2
...
C!! changed
      ...
      smrn = 0.
C
C      if(diag) write(99,'(" opts: compound=",L3,", basefl=",L3,", rain="
C     &,L3)') cmpd, bflow, chrain
!**
      IF( nprint .EQ. 5)
...
C            if(diag.and. j .eq. jd) write(99,'("ql ",i3, 6g12.4)') j, 
C     &  qlat(1), qnet, ewpr1(j,1), flm, hm, h2n
C  
          end do
...
C          if(diag) write(99,'(" zoneA ",8g13.5)') 
C     &  ewpr1(2,1),rf(2,1),fav(2,1),qlat(1),qlat(2)
              go to 70
...
C  route flow for general case
            ...
            if (ierr .gt. 0) then
C              if(diag)  write(99,'(I2,6g13.4)') 
C     &               j,h1(j,1),h1(jp1,1),h2(j),fav(j,1)
              call errxit(msg(ms:18), 'no convergence (errch2)')
            else if (j .eq. jd) then  !  
              itrt = itrt - ierr
C              if(diag .and. j .eq. jd) 
C     &     write(99,177) j,h1(j,1),h2(j),h1(jp1,1),h2jp1,fav(jp1,1)
C 177  format(" J    h1(j,1)      h2(j,1)       h1(jp1,1)      h2(jp)    
C     &   fav(jp1)"/I2,6g13.4)
            end if
...
C              write(99,'("  Diagnostic arrays:")')
C              write(99,992)'h2  ',(h2(j),j=1,nk)
C              write(99,992)'q2  ',(q2(j,1),j=1,nk)
...
C              write(99,993)jd,ul(units),ul(units),ul(units),tmi,rf(jd,1)
C     &         ,fav(jd,1), q2(jd,1), ql(jd,1), qlat(1) !, sov
C             write(99,991)jd, itrt, qbal(7)*dx, vup, vstor, tbal,qlat(1)
C     &          ,qlat(2),smrn
C  993  format(1x,I2," t(min)      rf(",a1,"/s)     f(",a1,"/s)      q2("     
C     &,a1,"^3)     ql         qlat"/(8g12.4))
C  992  format(1x,a4,(8g12.4))
C  991  format(' j itr    Qb7          SumIn        stor         bal     
C     &  QL1         QL2'/1x,i2,i4,(8g12.4))
            end if
C - - - - - - - - - - - -
C                                                                      sediment
C------------------------------------------------------------------------------
...
C        write(99,'(/I4," resets of dt by courant criterea. ")') iretn
      end if
C
      if(vtn .gt. 0.) then
       avwr = swr/vtn
...
C------------------------------------------------------------------------------
      ...
      call qwrt (id, nchan, msg, qp, tp, vin,
