...
C------------------------------------------------------------------------------
     ...
     &        outq(2), i, idup, upr, outr, latq, out999, up888,
     &        nr, nq !units, id,  
C**
...
C** 22/4/03
        if(n .le. 2 .and. dx .ge. dxlim .and. lwarn) then
          lwarn = .false.
C          write(files(3), 1990) id, dx, chl, n!  , dxlim
C 1990 format(/" Urban Element",i3,":  based on its length and parameter 
C     &CLEN, the"/"  numerical increment of ",f6.1,1x,a2," for fraction "
C     &,i1," is too large for realistic",/"  solution of the flow equatio
C     &n, and may give  misleading results."/)
        end if
 !     
...
C------------------------------------------------------------------------------
        ...
        vcm = 0.
        dfir = dint
C        if(diag) write(99,'(" c,d",2f12.6)') covr, dint
...
C                                                               store discharge
          ...
          if(diag) then
C            write(99,'(" n,q: ",I2,3g12.4)') n, dint, rfu, q2(nk)
C            write(99,'(" iter2: ",i4, g12.4)') itot, h1r
C            write(99,'("  ql",8g12.4)') (ql(j),j=1,nk)
...
C**  22/4/03 dx check
      if(dx .ge. dxlim) then
C        write(files(3), 1991) id, dx, chl!  , dxlim
C 1991 format(/"   Urban street ",i3,":  based on its length and paramete
C     &r CLEN,"/"  the numerical increment of ",f6.1,1x,a2," is too large
C     & for",/"  realistic numerical solution of the flow equation, and m
C     &ay "/"  give  misleading results."/)
      end if
 !     
...
C**  this is a dummy call to put in zeros
        call swrt (wso, 0., 0., 0., 0., 0., outc, wse_neg, wse_pos)
      end if
