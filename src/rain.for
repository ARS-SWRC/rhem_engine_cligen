...
C------------------------------------------------------------------------------
      ...
      subroutine rain ( filen, idevent, iday, imon, iyear, tfins,
     &                  duracion, lluvia, mxint )  !, units, limit, delt
...
C ---- CLIGEN variables -------------------------------------------------------	 
      integer  ibrkpt, ninten(1000), norain(1000),nplane,iplane,mxplan
      integer  idevent, iday, imon, iyear
      
      real     dur, intsty(1000), timem(1000), ip, timep, p, duracion
      real     rainfall(1000), prcp, stmdur, mxint, deltfq, fq, lluvia
      real     intdl(21), timedl(21)
C -----------------------------------------------------------------------------	  

      dimension t(1000), z(1000), ti(100000), zi(100000), x(100), 
     &      y(100), a(100), b(100), d(100), nd(100), si(100),
     &      indx(100), endx(100)

C - CLIGEN varibales istrng stmfil	 
      character rlab*1, gageid*10, msg*20

      save nrg, ti, zi, nd, x, y, a, b, d, kr, indx, endx, si
C
      save rainfall, prcp, dur, intsty, stmdur, timem, ninten,
     &     ip, timep, p 
C------------------------------------------------------------------------------
C      tfins = ( stmdur * 3600. ) * 1.2 ! end time in seconds
C      tfins = tfin*60.  ! end time in seconds
C      if (filen .eq. -1) then
...
C------------------------------------------------------------------------------
C-------Mariano Hernandez 07Nov2012 -------------------------------------------
C       Read data from the CLIGEN output

C        call open ( filen, 2, 65, fname(2) )
C        call eatcom ( filen )
C        read ( filen, * ) nevent
C        read ( filen, * ) ibrkpt
C
...
C           if  ( nevent .gt. 1 ) then
          ...
          call stmget( idevent, iday, imon, iyear, ibrkpt, mxint,
     &                dur, intsty, timem, ninten, ip, timep, p, norain,
     &                prcp, rainfall, stmdur, nplane, mxplan, filen )
...
C           if ( ibrkpt .eq. 0)
          call disag( dur, intsty, timem, ninten, intdl, timedl, 
     &                deltfq, fq, p, ip, timep, rainfall, stmdur,
     &                iplane, mxplan )	 
C          end if
C100     continue
C -----------------------------------------------------------------------------
C------------------------------------------------------------------------------
      tfins =  stmdur * 2. ! end time in seconds
C      tfins = tfin*60.  ! end time in seconds
      if (filen .eq. -1) then
C                                               no rainfall data -- assume zero
        nrg = 1
...
C                                             read data sets for all rain gages
C------------------------------------------------------------------------------
C        call reader (filen, gageid, ierr)
        np = ninten(1)
        rlab = 'I'
        ierr = 0
...
!              call reportw
C              if(errxt) 
C     & call errxit(fname(2), ' eof encountered in rainfile')
            end if
            go to 8
...
C        if (i .eq. 2 .and. .not. xy)
C     &  call errxit (msg, 'x or y coordinate missing or invalid')

C        msg = 'rain gage '//gageid
C                                                                       x coord
C        call getr4 ('X', 0, x(i), ierr)

C        if (ierr .gt. 0) then

C          if (i .ge. 2) then

C            call errxit (msg, 'x coordinate missing or invalid')
...
C        end if
C                                                                       y coord
C        call getr4 ('Y', 0, y(i), ierr)

C        if (ierr .gt. 0) then

C          if (i .ge. 2) then

C            call errxit (msg, 'y coordinate missing or invalid')
...
C        call getr4 ('S', 0, si(i), ierr)

C        if (ierr .gt. 0) si(i) = -1.
         si(1) = -1.		   
C                                                               # of data pairs
C        call geti4 ('N', 0, np, ierr)

C        if (ierr .gt. 0) call errxit
C     &  (msg, 'number of data pairs not specified')

C        np = ninten(1)
        t1 = -1.
C        rlab = 'D'

C        call getr4 (rlab, 1, zi(n+1), ierr)

C        if (ierr .gt. 0) rlab = 'I'
...
C                                                          data set in array ti
        ...
        do j = 1, np
C                                                                      get data
C          call getr4 ('T', j, t2, ierr)

C          if (ierr .gt. 0) call errxit
C     &    (msg, 'time value missing or invalid')

C          if (t2 .le. t1) call errxit
C     &    (msg, 'time not increasing')

          k = n + j

C          if (k .gt. 100000) stop ' error - too much rainfall data'
...
C          call getr4 (rlab, j, zi(k), ierr)

           zi(k) = intsty(j)*1000.*3600.

C          if (ierr .gt. 0) call errxit
C     &    (msg, 'depth/intensity value missing or invalid')
