...
C-------------------------------------------------------------------------------
      ...
      integer ierr, i, j, ms, n, nu, outr, outq, nr, nq, nc 
      integer upq(10), idup(10), upr(10), upc(10,5), outc(5)

      real qrain, qu_sum, qru, qu, cu, co, area, coarea, qs_sum(5)

      character msg*18
      ! character attr*2(5) Changed by BIO, gfortran error
     ...
     &  (msg(ms:18), 'upstream inflow not found')
        ...
        if (sed) then
          ...
          do n = 1, nps
            ...
            call old (idup(j), attr(n), upc(j,n), nc, ierr)
